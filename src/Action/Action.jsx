const Action = () => (
  <div className="flex flex-col justify-center items-center text-center w-screen h-1/3 gap-10">
    <h1 className="w-[75vw]">Arjun Liji - Official Website</h1>
    <p className="w-[90vw]">
      My name is Arjun M Liji, I am 18 currently pursuing my Bachelor's in Computer Science and Engineering at NSSCE, Palakkad.
      Thank you for visiting my site and if you are here, just chill just relax while I build this website further. Thank you for ur patience...
    </p>
    <div className="flex flex-wrap justify-center gap-5">
      <a href="/cv.pdf" className="px-7 py-3 border-2 border-white rounded-2xl">Download CV</a>
      <a href="/" className="px-7 py-3 border-2 border-white rounded-2xl">Gallery</a>
      <a href="/" className="px-7 py-3 border-3 border-white rounded-2xl">Contact</a>
    </div>
  </div>
);
export default Action;
